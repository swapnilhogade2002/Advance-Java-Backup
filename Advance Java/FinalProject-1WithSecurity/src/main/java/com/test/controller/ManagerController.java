...
//  download pdf
	...
	public void downloadEntities(HttpServletResponse response) throws IOException {
	    ...
	    // Write the headers to the response output stream with bold formatting
	    writer.write("ID,Name,Email,MobileNo,Password,Role,Gender,City\n");

	    // Write the data to the response output stream
	    for (Employee entity : entities) {
	        // Customize based on your entity structure
	        writer.write(entity.getId() + "," + entity.getName() + "," + entity.getEmail()
	                + "," + entity.getMobileNo() + "," + entity.getPassword()
	                + "," + entity.getRole() + "," + entity.getGender() + "," + entity.getCity() + "\n");
	    }

	    // Flush the writer to ensure all content is written to the response
	    writer.flush();
	}
